...
def valid(args, device, agent):
    ...
    with torch.no_grad():
        while not terminated:
            ...
            for i in range(len(cases)):
                if actions.runnable_cases[i]:
                    mac = actions.actions[i][0].item()
                    op = actions.actions[i][1].item()
                    duration_val = valid_env.state.operation_time[i, mac, op].item()
                    action_list[i].append([mac, op, cur_time, duration_val])
            if gain is None:
                gain = reward
            else:
                gain = gain + reward
    ...
    max_time_tensor, index = operation_finished_time_earliest.max(dim=-1)
    makespans = max_time_tensor.reshape(-1).tolist()
    max_time = max_time_tensor.float().mean()
    return max_time.item(), (gain[0].item(), gain[1].item()), action_list, makespans


def eval(args, device):
    agent = PPO(args, device=device)
    agent.load_policy(args['save_path'] + args['checkpoint_id'])
    avg_makespan, avg_gain, action_list, makespans = valid(args, device, agent)
    print(f'average markspan {avg_makespan}')
    return avg_makespan, action_list, makespans
